...
#include "D3.h"
...
#if defined(D3__PC)
    #define D3_SetP4PinDriveMode(shift, mode)  \
    do { \
        D3_PC =   (D3_PC & \
                                (uint32)(~(uint32)(D3_DRIVE_MODE_IND_MASK << \
                                (D3_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (D3_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define D3_SetP4PinDriveMode(shift, mode)  \
        do { \
            D3_USBIO_CTRL_REG = (D3_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(D3_DRIVE_MODE_IND_MASK << \
                                    (D3_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (D3_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		D3_SetP4PinDriveMode(D3__0__SHIFT, mode);
    }
#endif
...
void D3_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  D3_INTCFG & (uint32)(~(uint32)position);
    D3_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}

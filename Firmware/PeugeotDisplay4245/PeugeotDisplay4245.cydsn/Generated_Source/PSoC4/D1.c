...
#include "D1.h"
...
#if defined(D1__PC)
    #define D1_SetP4PinDriveMode(shift, mode)  \
    do { \
        D1_PC =   (D1_PC & \
                                (uint32)(~(uint32)(D1_DRIVE_MODE_IND_MASK << \
                                (D1_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (D1_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define D1_SetP4PinDriveMode(shift, mode)  \
        do { \
            D1_USBIO_CTRL_REG = (D1_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(D1_DRIVE_MODE_IND_MASK << \
                                    (D1_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (D1_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		D1_SetP4PinDriveMode(D1__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(D1_DR & (uint8)(~D1_MASK));
    drVal = (drVal | ((uint8)(value << D1_SHIFT) & D1_MASK));
    D1_DR = (uint32)drVal;
}
...
uint8 D1_Read(void)
{
    return (uint8)((D1_PS & D1_MASK) >> D1_SHIFT);
}
...
{
    return (uint8)((D1_DR & D1_MASK) >> D1_SHIFT);
}
...
void D1_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  D1_INTCFG & (uint32)(~(uint32)position);
    D1_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(D1_INTSTAT & D1_MASK);
	D1_INTSTAT = maskedStatus;
    return maskedStatus >> D1_SHIFT;
}
